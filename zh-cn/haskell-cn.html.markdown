...
Haskell 被设计成一种实用的纯函数式编程语言。它因为 monads 及其类型系统而出名，但它的优雅才是我钟情于它的原因。Haskell 使得编程对于我而言是一种真正的快乐。
...
-- 因为 Haskell 是 “惰性求值” 的，所以无限元素的列表可以正常运作。这意味着
-- Haskell 可以只在它需要的时候计算。 因此你可以请求
-- 列表中的第 1000 个元素，Haskell 会返回给你

[1..] !! 999 -- 1000

-- Haskell 计算了列表中 1 - 1000 个元素...但是
-- 这个无限元素的列表中剩下的元素还不存在！如非必要， Haskell 不会
-- 真正地计算它们。
...
-- 模式匹配与上面类似。这里有三种不同的 fib 
-- 定义。Haskell 将自动调用第一个匹配模式值
-- 的函数。
fib 1 = 1
fib 2 = 2
...
-- 使用 fold（在一些语言称为`inject`），以一个匿名
-- 函数为参数。foldl1 意味着左折叠(fold left), 并且使用列表中第一个值
-- 作为累加器的初始值。
...
-- 并对结果乘以 5，之后返回最后的值。
-- 译者注：f(x) = 5x, g(x) = 10+x, 则 foo = f(x) . g(x) = f(g(x)) = 5(10+x)
foo = (*5) . (+10)
